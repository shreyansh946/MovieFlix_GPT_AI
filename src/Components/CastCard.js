...
import { IMG_CDN, USER_AVATAR } from "../Utilis/Constant"
    ...
    return (
        <div className="w-32 md:w-44 text-black m-3 shadow-lg rounded-xl">
            <div className="w-[100px] md:w-[120px]">
                {
                     img_path == null ? (
                       (<img className="rounded-t-lg object-cover" src={USER_AVATAR} alt="cast-img" />)
                    ) 
                    :
                    (
                        <img  
                            className="rounded-t-lg object-cover"
                        src={IMG_CDN + img_path} alt="cast-img" loading="lazy" />
                    )
                }
            </div>
